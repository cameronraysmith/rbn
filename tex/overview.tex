...
\end{equation}
Here $R_\alpha$ is shorthand for $R(C_\alpha,S,\mu_\alpha,\tau^{(1)}_\alpha)$ and $R$ is shorthand for $R(G,S,\mu,\tau^{(1)})$. \ref{eq:robschematic} demonstrates just as stability decomposes over the SCCs of a network architecture, which was shown in the previous section, so does dynamical robustness.

\subsection{Example computation of dynamical robustness}
Before proceeding to demonstrate which network architectures maximize dynamical robustness we illustrate how the result of the previous section applies to simple examples. Dynamical robustness can be computed for systems like those that derive from the sampling of Jacobian matrices shown in \ref{fig:robustnessconcept} and \ref{fig:jacobianvectorfields}. \ref{eq:robschematic} is a schematized version of \ref{eq:robustnessbygraph} for the case of perturbing a single element at a time.  \reffigscc$\,$ shows examples demonstrating the usage of this expression. For instance, if our graph is the one in \reffigscc{} (middle panels), then we have two connected components, one with two nodes, and one with one node.  From \ref{tab:structstabmat}, we know that the graph with two nodes has probability $0.25$ of being stable and robustness $0.62$.  The graph with one node corresponds to a $1 \times 1$ matrix, so we have probability $0.5$ of stability and robustness $0.5$.  Thus, the probability of our 3-node graph being stable is $0.5 \times 0.25 = 0.125$ and its robustness is computed from \ref{eq:robschematic} in \reffigscc{}, which agrees with the value computed in \ref{tab:structstabmat3} up to sampling error.

Due to the symmetries of robustness, there are two more network architectures having the same SCCs but different dynamical robustness values. Their values for dynamical robustness are given for $l=0,1,2$ in
$$
...
Following the examples of the previous section, we now show, in general, that dynamical robustness is maximized for a given collection of SCCs by maximizing the number of links between them.  The proof consists of two stages; deriving an upper bound on the robustness of a network in terms of its SCCs and then showing that this bound is attained by a graph if and only if it maximizes the number of links between its SCCs.

Suppose we have a directed graph $G$ with SCCs $C_1,\ldots,C_n$.  Since every acyclic digraph can be embedded into a totally ordered set, we may assume without loss of generality that our components have been ordered in a way such that, if $(\alpha,\beta) \in \hier(G)$, then $\alpha < \beta$.  Given two connected components $C_{\alpha}$ and $C_{\beta}$ with $v_{\alpha}$ and $v_{\beta}$ nodes respectively, we have a maximum of $v_{\alpha} v_{\beta}$ links going from $C_{\alpha}$ to $C_{\beta}$.  Hence, $l \le l_{\mathrm{max}}$ where
...
v_{\alpha}^2.$$
Examining the expression in \ref{eq:robschematic} and noting that $R_{\alpha}$ are all strictly less than one leads to the conclusion that networks maximizing $l$ for a given set of SCCs, will also maximize $R$.  Hence, we have $R(G,S,\mu,\tau) \le R_{\mathrm{max}}$ where $R_{\mathrm{max}}$ is equivalent to \ref{eq:robschematic} with $l_{\mathrm{max}}$ substituted for $l$:
\begin{widetext}
...
Let $G_{\mathrm{tot}}$ refer to the directed graph on $n$ nodes with a link from node $i$ to node $j$ whenever $i < j$ and let $G_{\mathrm{max}}$ refer to the directed graph whose SCCs are $C_1, \ldots, C_n$, but which includes all possible links between each pair of components $C_\alpha, C_\beta$ when $\alpha < \beta$.  Then $\hier (G_{\mathrm{max}}) = G_{\mathrm{tot}}$ and $R(G_{\mathrm{max}}) = R_{\mathrm{max}}$.  Thus, we see that our bound $R_{\mathrm{max}}$ is attained by $G_{\mathrm{max}}$ so conclude that $R_{\mathrm{max}}$ is indeed the maximum value of dynamical robustness for any G-class of fixed points.  Furthermore, we note that for any other graph $G$, which has the maximum number of links between its SCCs, we also have $R(G) = R_{\mathrm{max}}$. By contrast, for any graph $G$, which has less than the maximum number of links between its SCCs, we have $R(G) < R_{\mathrm{max}}$.

\subsection{Simultaneous perturbation of multiple interaction strengths}
The results of the previous sections also hold when relationships between multiple system variables are perturbed simultaneously,
although the notation becomes more complicated.  Suppose that we
resample $m$ interactions, as a model for cases in which multiple interaction strengths among system variables are modified in a single timestep of the evolutionary process.  Then the analog of \ref{eq:singleresamplemutau} is
...
m_0, m_1, \ldots. m_n}$ ways of choosing $m_i$ links from within $C_i$ and
...
This analytical result implies that the interaction graphs for systems that are the most robust will maximize the number of links between a given set of SCCs. Because this result is purely topological in nature, it does not depend at all upon any particular details such as the size of the system or the probability distribution from which the variable interaction strengths are sampled. Any network whose associated dynamical system has the interaction graph among its SCCs equivalent to the total ordering is more robust than those associated to any of the other interaction graphs in \reffigscc{}. The graph associated to the total ordering is the most hierarchical network architecture for any given number of SCCs like that of \reffigscc{} top for three variable systems where the \emph{highest} variable in the hierarchy has directed links to all other variables in the network, the second highest variable has directed links to all other variables in the network except the highest one, \emph{et cetera} (\refsupp{} \ref{sec:totalordering}). The result that dynamical robustness is correlated with network hierarchy therefore applies to an even broader class of dynamical systems than the particular random ensembles we have studied directly.

\subsection{The most dynamically robust network architectures have a single type of SCC}
The previously considered optimization problem maximized dynamical robustness subject to the constraint that the network have a particular set of SCCs.  We will now generalize this to situations in which we fix the types of SCCs which may appear in our network, but allow arbitrary numbers of each type.  We will study the constrained optimization problem where we fix the total number of nodes to constant $N$. This analysis will show that the most dynamically robust networks will be homogeneous in the types of their SCCs thereby having only a single type. This result drastically limits the number of different networks on $N$ nodes that are candidates for being the most dynamically robust.

Suppose that ${\hat C}_1, \ldots, {\hat C}_n$ is a collection of non-isomorphic strongly connected directed graphs and that $G$ is a directed graph such that every SCC of $G$ is isomorphic to some ${\hat C}_i$ for some $i$.  For all $i \in \{1, \ldots, n\}$, let $k_i$ be the number of SCCs of $G$ which are isomorphic to ${\hat C}_i$.  Then, following algebraic manipulation, \ref{eq:robschematic} can be rewritten as follows:
\begin{equation}\label{eq:robtype}
1-R(G) = \frac{\sum_{i=1}^n k_i d_i (1-R_i)}{l + \sum_{i=1}^n k_i d_i}.
\end{equation}
Maximizing $R$ is equivalent to minimizing $1-R$.  Even though the $k_i$'s are integer variables, we will approximate them as positive real variables.  The accuracy of this approximation increases in the limit of large networks.

When maximizing robustness subject to a fixed number $N$ of nodes, we can make use of our previous result in \ref{eq:sccmaxrobustness} to restrict attention to directed graphs with the maximal number of links between SCCs.  Hence, we can replace $l$ with $l_{\mathrm{max}}$ in \ref{eq:robtype} to obtain
\begin{equation}
...
\begin{equation}\label{eq:robqual}
 ...
 \frac{2 d_i (1 - R_i)}
      {(k_i - 1) v_i^2 + 2 d_i},
\end{equation}
hence the network which maximizes robustness will have as many SCCs as possible be of the type which have the smallest value of $q_i$.
