...
$$
r = \frac{\sum_{t=0}^{T-1} \mathbf{1}_S (s_{t+1}^T) \mathbf{1}_S (s_{t}^T)}{\sum_{t=0}^{T-1} \mathbf{1}_S (s_{t}^T)},
$$
where we have introduced the indicator function $\mathbf{1}_S$ on $\mathcal{S}$ such that $\mathbf{1}_S(S) = 1$ and $\mathbf{1}_S(U) = 0$.
If the process $P(s^T)$ has limits $\mu(s) = \lim_{t \rightarrow \infty} P(s_t^T=s)$ and $\tau(s' \mid s) = \lim_{t \rightarrow \infty} P(s_{t+1}^T = s' \mid s_t^T = s)$ for $s,s' \in \mathcal{S}$
%stationary probability $P^{\infty}(s)$ for $s \in \mathcal{S}$
, the expectation of $r$ as $T \rightarrow \infty$ is approximated by
$$
R(S,\mu,\tau) = E[r] \approx \frac{\sum_{s,s'} \mu(s) \tau(s' \mid s)}{\sum_{s=S} \mu(s)}.
$$
