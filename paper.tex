...
In order to discuss how different parts of the system influence each
...
A quality which is of interest to us is robustness or structural
...
$C_i$, the probability that the system will remain stable after
resamling is $R(\hbox{avg stab} C_i, 1)$.  If we resample a link
...
R(\hbox{avg stab} G, 1) =
\frac{\sum_{i,j \in \hier(G)} \ell_{ij} +
      \sum_{i=1}^n \connectivity_i R(C_i)}
     {\sum_{i,j \in \hier(G)} \ell_{ij} +
...
$$\sum_{i=1}^{n-1}\sum_{j=i+1}^{n}v_i v_j~=~\frac{1}{2} \left( \sum_{i=1}^{n} v_i \right)^2-\frac{1}{2} \sum_{i=1}^{n} v_i^2,$$
...
although the notation becomes more complicated.  Suppose that, as
before, we have a graph $G$ with strongly connected components $C_i$,
but that now we
resample $m$ nodes.  Then, given a partition $m = \sum_{i=0}^n m_i$
such that $m_0 \le \sum_{i=1}^{n-1}\sum_{j=i}^n \ell_{ij}$ and $m_i 
\le \connectivity_i$ when $1 \le i \le n$,
there are ${m \choose m_0, m_1, \ldots. m_n}$ ways of choosing $m_i$
links from $C_i$ and $m_0$ links between strongly connected
components.  Hence, if we denote the set of partitions which satisfy
our inequalities as $M$, our weighted average becomes
\begin{equation}
R(\hbox{avg stab} (G), m) =
\frac{\sum\limits_{(m_0, m_1, \ldots m_n) \in M} {m \choose m_0, m_1, \ldots. m_n}
      \left(m_0 + \sum_{i=1}^n m_i R(\hbox{avg stab} (C_i), m_i)\right)}
     {m \sum\limits_{(m_0, m_1, \ldots m_n) \in M} {m \choose m_0, m_1, \ldots. m_n}}
\end{equation}
