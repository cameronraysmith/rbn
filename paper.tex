...
This can be done in terms of conditions on the coefficients of the
characteristic polynomials associated to the systems described by matrices $A$ above \cite{Gantmacher1959}.

\section{Network hierarchy and strongly connected components}

Open systems are distinguished by having some of the variables be control variables whose values are given as specified functions of time rather than determined by the dynamics.  Given several such open systems, we may combine them to produce a larger system by setting the control variables of a subsystem equal to the dynamical variables of another system.  The composition and decomposition of open systems has algebraic structure equivalent to certain a kind of operad \cite{Vagner2014}.  Taking this notion to its logical extreme, one can dissect a system into a collection of one open system for each dynamical variable.  However, for most purposes, such a decomposition would be uninteresting and what we instead want is a decomposition into relatively self-contained modules.  One method of accomplishing this based upon the topology of the system graph is the decomposition into strongly connected components.
