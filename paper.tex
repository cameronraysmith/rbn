...
corresponds to a block triangular decomposition of matrices
describle by that digraph.  Say that the graph $G$ has strongly
connected components $C_1, C_2, \ldots C_n$, which have been
labelled in such a way that there are no links from vertices in
component $C_i$ to component $C_j$ when $i < j$.  Label the
vertices in such a way that $v_1, v_{n_1}$ belong to $C_1$,
$v_1, v_{n_1}$ belong to $C_2$, etc.  Then if $M$ is a matrix
consistent with $G$, it must be block triangular.  [explain better]

An important consequence of this is that edges which go between
different strongly connected components do not contribute to
determining stability.  Hence, if we resample a matrix entry
corresponding to such an edge, the conditional probability 
of the matrix remaining stable upon resampling is 1.  Hence, if
our graph $G$ has strongly connected components $C_1, C_2, \ldots C_n$
where $C_i$ has $v_i$ vertices and connectivity $c_1$ and there
are $\ell_{ij}$ links between components $i$ and $j$, then the
total robustness to resampling a random entry is
\[
R(G) = \frac{\sum_{i=1}^n \sum_{j=1}^n \ell_{ij} + \sum_{i=1}^n c_i R_i}
            {\sum_{i=1}^n \sum_{j=1}^n \ell_{ij} + \sum_{i=1}^n c_i}
\]
            
\[
R_{\mathrm{max}} = \frac{\frac{1}{2} ((\sum_i n_i)^2 - \sum_i n_i^2) + \sum_i c_i R_i}
            {\frac{1}{2} ((\sum_i n_i)^2 - \sum_i n_i^2) + \sum_i c_i}
\]
