...
\end{pmatrix}
$$
having connectivity $\connectivity$ equal to the number of edges of the graph, in this case $\connectivity = 3$. For the system $X = \{G_X, M_X, F_X\}$, where $M_X \equiv \{M_1,M_2\}$ and $F_X \equiv \{F_1,F_2\}$ such that $F_1$ is constrained to be a function of $x_2$ and $F_2$ is a function of both $x_1$ and $x_2$ yielding the flow equations
\begin{align*}
...
Suppose that $M$ is an $n \times n$ matrix with real coefficients. We will consider the case in which each of the parameters defining the matrix is sampled from the uniform distribution $\mathcal{U}(-1,1)$ so that the parameter space corresponds to the $\connectivity$-dimensional hypercube, $H^\connectivity$, of edge length $r=2$, centered about the origin, and having volume $r^d$.  We want to know the probability that, if we resample the $k^{th}$ element of this matrix, the resulting matrix will correspond to a stable matrix which is given by a special case of \ref{eq:robustness}
\begin{align}\label{eq:condprobgen}
 R (stab) =
  \frac{\int_{H^\connectivity} dx_1 \cdots dx_d \int_{-1}^{1} dy \, \mathbf{1}_{stab}(x_1 \cdots x_k \cdots x_d) \mathbf{1}_{stab}(x_1 \cdots y \cdots x_d)}
  {\int_{H^\connectivity} dx_1 \cdots dx_d  \, \mathbf{1}_{stab}(x_1 \cdots x_d)}
\end{align}

As previously stated, we call the matrix $M$ stable if all its eigenvalues have negative real part. Define the characteristic polynomial as $\chi(M)(z) = \det(zI - M)$. Denote the coefficients of $\chi(M)$ as $s_j$ and its roots as $r_j$, writing $p(x) = x^n + \sum_{k=1}^n s_k x^{n-k} = \prod_{k=1}^n (x-r_k)$.
...
Using this approach the probability of \emph{a priori} stability and of robustness for all two component systems is given in Table \ref{tab:structstabmat}. Similarly, the analogous results for all three component systems are computed using Monte Carlo integration and shown in \ref{tab:structstabmat3} and \ref{fig:stab3x3}.

\todo{Say a few words about Monte Carlo integration and error bars.}

\section{Dependency of robustness upon connectivity and cycle number}
...
The relationship between connectivity and robustness for two component systems is shown in \ref{tab:structstabmat} and likewise for three component systems in \ref{tab:structstabmat3} and \ref{fig:stab3x3}. If we average over the different classes of matrices for a given connectivity we see there is a correlation between connectivity and robustness demonstrated by the red dots in \ref{fig:stab3x3}.
...
An important consequence of this is that stability is invariant to perturbations to the edges that go between different strongly connected components.  Hence, if we resample a matrix entry corresponding to such an edge, the conditional probability of the matrix remaining stable upon resampling is 1.  If our graph $G$ has strongly connected components $C_1, C_2, \ldots C_n$ where $C_i$ has $v_i$ vertices and connectivity $c_1$ and there are $\ell_{ij}$ links between components $i$ and $j$, then the total robustness to resampling a random entry is
\begin{align}
R(G) = \frac{\sum_{i=1}^n \sum_{j=1}^n \ell_{ij} + \sum_{i=1}^n c_i R_i}
            {\sum_{i=1}^n \sum_{j=1}^n \ell_{ij} + \sum_{i=1}^n c_i}, \label{eq:sccrobustness}
\end{align}
where $R_i$ refers to the independent robustness of component $i$.
\begin{align}
...
\todo{Since there is no point in publishing proofs of results contemporary
with Darwin's Origin of Species, this section will be replaced with a
reference to Gantmacher.}
Suppose that $M$ is an $n \times n$ matrix with real coefficients.
As described in \ref{sec:stabanalbn}, we call
